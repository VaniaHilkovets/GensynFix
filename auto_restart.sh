...
# Проверка на tmux/screen, чтобы сессия не упала
...
kill_node_procs() {
  ...
  # Убийство по swarm.pem
  if [ -f "$SWARM_PEM" ]; then
    echo "[$(date)] Убиваем по swarm.pem..."
    fuser -k "$SWARM_PEM" 2>/dev/null || true
    sleep 1
  fi

  # Убийство по сокетам hivemind
  echo "[$(date)] Убиваем по /tmp/hivemind-p2pd-*.sock..."
  fuser -k /tmp/hivemind-p2pd-*.sock 2>/dev/null || true
  sleep 1

  # Лог перед kill
  echo "[$(date)] Процессы перед убийством:"
  ...
  # Цикл по ps (cwd поддир, comm фильтр, пропуск wrapper)
  while read -r pid comm ppid pcomm; do
    if [ -d "/proc/$pid/cwd" ] && [[ "$(readlink -f /proc/$pid/cwd)" == *"$SCRIPT_DIR"* ]]; then
      if [[ "$comm" =~ ^(python|python3|bash|sh|p2p|daemon|hivemind|p2pd|wandb|gpu_stats|torch_shm_manager)$ ]] && [ "$pid" != "$$" ] && [ "$pid" != "$SLEEP_GUARD_PID" ]; then
        ...
        echo "[$(date)] Пропускаем PID=$pid ($comm)"
      fi
    fi
  ...
  done || true)  # Игнор, если ps сломается

  # Дополнительный pkill
  echo "[$(date)] Дополнительно убиваем..."
...
while true; do
  ...
  export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True  # Анти-фрагментация для OOM

  ( sleep 1 && printf "n\n\n\n" ) | bash "$SCRIPT" 2>&1 | tee "$TMP_LOG" &
  PID=$!

  while kill -0 "$PID" 2>/dev/null || true; do  # Игнор, если PID мёртв
    sleep 5
    ...
    if [ -f "$TMP_LOG" ]; then
      ...
      if (( idle_time > MAX_IDLE )); then
        echo "[$(date)] Idle >10 мин. Перезапуск..."
        kill -9 "$PID" 2>/dev/null || true
        kill_node_procs
        break
      fi
    ...
    for ERR in "${KEYWORDS[@]}"; do
      if grep -E -q "$ERR" "$TMP_LOG" 2>/dev/null || true; then  # -E для regex
        echo "[$(date)] Ошибка '$ERR'. Перезапуск..."
        kill -9 "$PID" 2>/dev/null || true
        kill_node_procs
        break 2
      fi
  ...
  echo "[$(date)] Завершён. Перезапуск через 3 сек..."
  sleep 3
done
